...
$('#addButton').click(function(event){
  ...
  var group = $('.input-group:first'); //will need to only select first element! I like this better...
  ...
  var numberOptions = $('input').length; 
  ...
  newLabel.attr('for','option'+numberOptions);
  copy.children('input').attr('id','option'+numberOptions);
  ...
  var removeButton = copy.find('button:last-child');
  ...
  removeButton.click(function(event){
    ...
    whichButton.parents('.input-group').remove();
...
Add an event listener to this new button so that when clicked it:
  1. Selects the clicked input-group and the one below it (separtely)
  2. Swaps the location of those elements

To make this really work, you'll also need to re-number the elements, 
and move the red "X" button if you moved the first input.
*/
